...
export class AddbookComponent implements OnInit {
  ...
  bookName = new FormControl(this.addbooks.bookName ,[ Validators.required,
    Validators.minLength(8),
    Validators.pattern("[a-zA-Z ]*"),
  ]);
  bookAuthor = new FormControl(this.addbooks.bookAuthor,[ Validators.required,
    Validators.minLength(8),
    Validators.pattern("[a-zA-Z ]*"),
  ]);
  bookPrice=new FormControl(this.addbooks.bookPrice,[ Validators.required,
    Validators.minLength(1),
    Validators.pattern("[0-9 ]*"),
  ]);
  noOfBooks=new FormControl(this.addbooks.noOfBooks,[ Validators.required,
    Validators.minLength(1),
    Validators.pattern("[0-9]*"),
  ]);
  bookDescription=new FormControl(this.addbooks.bookDescription,[ Validators.required,
    Validators.minLength(8),
    Validators.pattern("[a-zA-Z ]*"),
  ]);
  constructor( 
          ...
          this.bookservice.addBook(this.addbooks).subscribe(
            (response:any) => {
             ...
             file.append('file', this.selectedFile, this.selectedFile.name);
              this.http.post("http://localhost:8080/book/uploadbookimage/"+response.obj.bookId, file,{ headers: new HttpHeaders().set('token', localStorage.getItem('token')) },).subscribe
              ((response:any) => {
